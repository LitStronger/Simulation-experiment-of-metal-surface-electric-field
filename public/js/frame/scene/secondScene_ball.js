var secondScene_ball = function (aeroplane = "square") {
...
function addBallPanel(aeroplane){
    wave = ""
    type = "0"
    radar = "433MHz"
    let columns = [
    ...
    let panel_l = downFormitem_s(clickFlag, columns_radar, "开始实验", "雷 达 类 型", 30, 505, "300px","175px", "left", "top", onRatioClick = (value) => {
        ...
        if(clickFlag == true){
            ...
            setTimeout(() => {
                ...
                //video_2 = addPicture("exp_2_2", aeroplane, radar, type, x = 300, y = 80, z = -20);
                clickFlag = true;
    ...
    let panel_2 = addAngleSlider_2(columns_angle,"选 择 角 度", 30 , 320, "left", "top", onRatioClick = (index) =>{
        ...
        if(index == 0){
            ...
            //obj.position = new BABYLON.Vector3(0,15,200);
            //radar.rotation = new BABYLON.Vector3(0 ,Math.PI, 0);
            //camera.alpha = 
            //camera.useAutoRotationBehavior = true;
            //camera.autoRotationBehavior.idleRotationSpeed = Math.PI;
           /* setTimeout(()=>{
                camera.useAutoRotationBehavior = false;
                console.log(camera.useAutoRotationBehavior)
            },550) */
        }else if(index == 1){
            type = "45"
            obj.rotation.x = Math.PI * 0.7 + Math.PI / 4;
            //obj.position = new BABYLON.Vector3(-141,15,141);
            //radar.rotation = new BABYLON.Vector3(0, Math.PI *(3/4), 0);
        }else if(index == 2){
            type = "90"
            obj.rotation.x =  Math.PI * 0.7 + Math.PI / 2;
            //obj.position = new BABYLON.Vector3(-200,15,0);
            //radar.rotation = new BABYLON.Vector3(0, Math.PI / 2, 0);
        }else if(index == 3){
            type = "135"
            obj.rotation.x =  Math.PI * 0.7 + Math.PI * (3/4);
            //obj.position = new BABYLON.Vector3(-141,15,-141);
            //radar.rotation = new BABYLON.Vector3(0, Math.PI / 4, 0);
        }else if(index == 4){
            obj.rotation.x =  Math.PI * 0.7 + Math.PI;
            //obj.position = new BABYLON.Vector3(0,15,-200);
            //radar.rotation = new BABYLON.Vector3(0,0,0);
        }
    })
